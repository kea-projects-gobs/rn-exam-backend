package dk.kea.rnexambackend.security.config;

import com.nimbusds.jose.jwk.source.ImmutableSecret;
import dk.kea.rnexambackend.security.error.CustomOAuth2AccessDeniedHandler;
import dk.kea.rnexambackend.security.error.CustomOAuth2AuthenticationEntryPoint;
...
public class SecurityConfig {
  ...
  public SecurityFilterChain securityFilterChain(HttpSecurity http, HandlerMappingIntrospector introspector) throws Exception {
    ...
    http.authorizeHttpRequests((authorize) -> authorize
            ...
            .requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.POST, "/api/auth/register")).permitAll() //Clients can create a user for themselves

            // Anonymous users can access these endpoints
            //.requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.GET, "/randomendpoint")).permitAll()

            // Only USERS should see their own user information and mealplans etc.
            //.requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.POST, "/randomendpoint")).hasAuthority("USER")


            // Eventual ADMIN logic
            //.requestMatchers(mvcMatcherBuilder.pattern(HttpMethod.POST, "/randomendpoint")).hasAuthority("ADMIN")
